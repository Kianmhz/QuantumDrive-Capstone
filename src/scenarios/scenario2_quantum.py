...
# --- Parameters (same as classical version) --------------------------------
TOWN = "Town04"
EGO_SPEED_MS = 18.0
NPC_SPEED_MS = 24.0
CUTIN_DELAY_S = 2.0
...
def ego_evasion_control(ego, action, current_speed):
    ...
    if action == "evasive_left":
        # Evasive left turn with reduced speed
        ego.apply_control(
...
def evaluate_action_cost(
    ego, npc, distance, longitudinal, lateral, ego_speed, npc_speed, action
):
    """Evaluate the cost of taking a specific action in the current scenario"""
    ...
    # Determine if NPC is cutting in
    is_cutting_in = (
    ...
    # Base cost depends on scenario
    base_cost = 0.0

    # Higher cost for dangerous situations
    if is_in_lane and longitudinal > 0 and distance < CRITICAL_DISTANCE:
        base_cost = 100.0  # Very dangerous situation
    elif is_cutting_in and longitudinal > 0 and distance < WARNING_DISTANCE:
        if ttc < 2.0:
            base_cost = 80.0  # Critical cut-in
        else:
            base_cost = 50.0  # Moderate cut-in
    elif is_ahead_close:
        base_cost = 30.0  # Vehicle ahead
    elif is_cutting_in:
        base_cost = 20.0  # Potential cut-in
    else:
        base_cost = 5.0  # Safe situation

    # Action-specific costs
    if action == "keep":
        # Keeping speed is good in safe situations but bad in dangerous ones
        if base_cost > 50:
            return base_cost * 1.5  # Penalty for not reacting to danger
        else:
            return base_cost * 0.5  # Reward for maintaining speed when safe

    elif action == "gentle_brake":
        # Gentle braking is good for moderate situations
        if 20 < base_cost < 80:
            return base_cost * 0.7  # Good for moderate situations
        else:
            return base_cost * 1.2  # Less effective in extreme situations

    elif action == "hard_brake":
        # Hard braking is good for critical situations but inefficient otherwise
        if base_cost > 50:
            return base_cost * 0.6  # Good for critical situations
        else:
            return base_cost * 1.8  # Penalty for overreacting

    elif action == "evasive_left":
        # Evasive action is good for critical situations but has high cost
        if base_cost > 70:
            return base_cost * 0.8  # Good for very critical situations
        else:
            return base_cost * 2.0  # High penalty for unnecessary evasive action
...
def quantum_decision(ego, npc, distance, longitudinal, lateral, ego_speed, npc_speed):
    ...
    costs = {}
    for action in BIT_TO_ACTION.values():
        costs[action] = evaluate_action_cost(
            ego, npc, distance, longitudinal, lateral, ego_speed, npc_speed, action
        )
    ...
    return best_action, costs, counts
...
def main():
    ...
    try:
        ...
        print("[Quantum] HARD early cut-in in front of ego")
        ...
        for tick in range(ticks_total):
            ...
            if sim_time - last_snapshot_time >= SNAPSHOT_INTERVAL:
                save_snapshot(world, ego, npc, sim_time, "quantum")
                last_snapshot_time = sim_time

            # --- NPC Logic (same as classical) ---
            ...
            if tick >= reaction_tick:
                action, costs, counts = quantum_decision(
                ...
                if action != "keep":
                    print(
                        f"[Quantum Decision] {action} - Distance: {distance:.2f}m, Longitudinal: {longitudinal:.2f}m, Lateral: {lateral:.2f}m"
                    )
                    print(f"  Costs: {costs}")
                    print(f"  Counts: {counts}")

                # Track quantum decisions for analysis
                ...
                # Apply the action using the evasion control function
                ego_evasion_control(ego, action, ego_speed)
            else:
                ego_speed_control(ego, ego_speed, EGO_SPEED_MS)

        print("\n[Scenario Done] Quantum cut-in complete!")

        # Save quantum decision data
        with open("quantum_decisions.json", "w") as f:
            json.dump(quantum_decisions, f, indent=2)
